...
// Import the actual RPG_MENU_TREE
// Note: We'll use require to avoid module resolution issues
const { RPG_MENU_TREE } = require("../src/app/rpg/input/menuTree");
...
function generateActionHandlers() {
  ...
  RPG_MENU_TREE.forEach((menu: any) => {
    menu.subMenus.forEach((subMenu: any) => {
      subMenu.actions.forEach((action: any) => {
        ...
        // Skip if we've already generated this handler
...
${Array.from(handlerNames).map(name => `  ${name},`).join('\n')}
};
`;

  const outputPath = path.join(OUTPUT_DIR, 'actionHandlers.ts');
  fs.writeFileSync(outputPath, content);
  console.log(`✓ Generated ${handlerNames.size} action handlers: ${outputPath}`);
}
...
async function generateAll() {
  ...
  RPG_MENU_TREE.forEach((menu: any) => {
    menu.subMenus.forEach((subMenu: any) => {
      subMenu.tools.forEach((tool: any) => {
